...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        for (int i = 0; i < tableau_noeud1.length; i++) {
            ChoixBDD.insertNoeud(tableau_noeud1[i]);
        }
        for (int i = 0; i < tableau_choix1.length; i++) {
            ChoixBDD.insertChoix(tableau_choix1[i]);
        }
        ...
        if (v.getId() == R.id.bouton_next) {
            if (bouton_next.getText()=="Next") {
                ...
                if (Noeud_en_cours.getChoix()[1] == 0) {
                    ...
                    bouton_next.setText("Prendre une décision");
                }
            }else if (bouton_next.getText() == "Prendre une décision"){
                LancerActiviteDecision(Noeud_en_cours);
            }
        }
    }
    public void LancerActiviteDecision(Noeud Noeud_en_cours){
        // Cette fonction lance l'activité des decisions
        Intent intent = new Intent(this, ActivityDecision.class);
        intent.putExtra("Noeud", Noeud_en_cours.getId());
        // Ici on lance l'activité en transférant la valeur de possible_continuer
        // Et on demande un retour de va_continuer
        startActivityForResult(intent, choix_joueur);
    }
}
